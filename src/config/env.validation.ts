...
export function validateEnvironment(
  ...
  if (
    typeof telegramBotApiKey !== 'string' ||
    telegramBotApiKey.trim().length === 0
  ) {
    throw new Error(
      'Missing required environment variable: TELEGRAM_BOT_API_KEY',
    );
  }

  if (
    typeof telegramBotUsername !== 'string' ||
    telegramBotUsername.trim().length === 0
  ) {
    throw new Error(
      'Missing required environment variable: TELEGRAM_BOT_USERNAME',
    );
  }
  ...
  if (parsedAdminIds.length === 0) {
    throw new Error(
      'ADMIN_IDS must contain at least one valid numeric Telegram user ID',
    );
  }
